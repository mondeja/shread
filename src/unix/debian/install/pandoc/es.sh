...
function getPandocLatestVersion() {
  ...
  if [ $_PANDOC_RELEASES_INFO_MESSAGE_EXIT_CODE -eq 0 ]; then
    ...
    printf "\nOcurrió un error obteniendo la información de las releases de Pandoc" >&2
    printf " Compose de la API de Github.\nUrl: $_GET_PANDOC_LATEST_VERSION_URL\n" >&2
    printf "Error: $_PANDOC_RELEASES_INFO_MESSAGE\n" >&2
  ...
  if [ $_GET_PANDOC_LATEST_VERSION_EXIT_CODE -ne 0 ]; then
    ...
    if [ $_GET_PANDOC_LATEST_VERSION_ATTEMPTS -ge $_GET_PANDOC_LATEST_VERSION_MAX_ATTEMPTS ]; then
      printf "\e[91m\xE2\x9C\x95\e[39m\n" >&2
      print "\nOcurrió un error obteniendo la última versión de Pandoc.\n" >&2
      printf "Url: $_GET_PANDOC_LATEST_VERSION_URL\n" >&2
      printf "Código de error: $_GET_PANDOC_LATEST_VERSION_EXIT_CODE\n" >&2
      printf "Error: $_PANDOC_LASTEST_VERSION"
...
function downloadPandoc() {
  ...
  if  [ $_DOWNLOAD_PANDOC_EXIT_CODE -ne 0 ]; then
    printf "\e[91m\xE2\x9C\x95\e[39m\n" >&2
    printf "\nOcurrió un error con la descarga de Pandoc v$_PANDOC_LASTEST_VERSION\n" >&2
    printf "Url: $_DOWNLOAD_PANDOC_URL\n" >&2
    printf "Error: $_DOWNLOAD_PANDOC_OUTPUT\n" >&2
  ...
  if [ "$_PANDOC_BINARY_CONTENT" = "Not Found" ]; then
    ...
    if [ $_GET_PANDOC_VERSION_404_ATTEMPTS -ge $_GET_PANDOC_VERSION_404_MAX_ATTEMPTS ]; then
      ...
      printf "Error: $_PANDOC_BINARY_CONTENT\n" >&2
      exit 404
    else
...
if [ "$_PANDOC_FILEPATH" = "" ]; then
  ...
  if [ $_PANDOC_VERSION_OUTPUT_EXIT_CODE -ne 0 ]; then
    ...
    printPrependedStdout
  	printf "  Encontrado Pandoc v$_PANDOC_INSTALLED_VERSION instalado en el sistema"

    # Si no está actualizado a la última versión
    if [ "$_PANDOC_INSTALLED_VERSION" != "$_PANDOC_LASTEST_VERSION" ]; then
      printf " \e[92m\xE2\x9C\x94\e[39m\n"
      printPrependedStdout
      printf "  Actualizando Pandoc (v$_PANDOC_INSTALLED_VERSION"
