...
function getGoogleDriveLatestVersion() {
  _GDRIVE_RELEASES_INFO=$(curl -s $_GET_GDRIVE_LATEST_VERSION_URL 2>&1)
  ...
  if [ $_GDRIVE_RELEASES_INFO_MESSAGE_EXIT_CODE -eq 0 ]; then
    ...
    printf "\n$_MSG_ERROR_RETRIEVING_GDRIVE_RELEASES\n" >&2
    printf "$_MSG_URL: $_GET_GDRIVE_LATEST_VERSION_URL\n" >&2
    printf "$_MSG_ERROR: $_GDRIVE_RELEASES_INFO_MESSAGE\n" >&2
  ...
  if [ $_PARSE_GDRIVE_LASTEST_VERSION_EXIT_CODE -ne 0 ]; then
    printf "\e[91m\xE2\x9C\x95\e[39m\n" >&2
    printf "\n$_MSG_ERROR_PARSING_PENULTIMATE_GDRIVE_RELEASES_VERSION\n" >&2
    printf "$_MSG_URL: $_GET_GDRIVE_LATEST_VERSION_URL\n"
    printf "$_MSG_ERROR_CODE: $_PARSE_GDRIVE_LASTEST_VERSION_EXIT_CODE\n" >&2
    printf "$_MSG_ERROR: $_PARSE_GDRIVE_LASTEST_VERSION"
  ...
  if [ $_GET_GDRIVE_LATEST_VERSION_EXIT_CODE -ne 0 ]; then
    let "_GET_GDRIVE_LATEST_VERSION_ATTEMPTS++"
    if [ $_GET_GDRIVE_LATEST_VERSION_ATTEMPTS -ge $_GET_GDRIVE_LATEST_VERSION_MAX_ATTEMPTS ]; then
      printf "\e[91m\xE2\x9C\x95\e[39m\n" >&2
      print "\n$_MSG_ERROR_RETRIEVING_LAST_GDRIVE_RELEASE_VERSION\n" >&2
      printf "$_MSG_URL: $_GET_GDRIVE_LATEST_VERSION_URL\n" >&2
      printf "$_MSG_ERROR_CODE: $_GET_GDRIVE_LATEST_VERSION_EXIT_CODE\n" >&2
      printf "$_MSG_ERROR: $_GDRIVE_LASTEST_VERSION"
  ...
  if [ $_DOWNLOAD_GDRIVE_URL_EXISTS -ne 1 ]; then
    let "_GET_GDRIVE_VERSION_404_ATTEMPTS++"
    let "_GET_GDRIVE_VERSION_INDEX++"
    getGoogleDriveLatestVersion
  else
    printf " (v$_GDRIVE_LASTEST_VERSION) \e[92m\xE2\x9C\x94\e[39m\n"
  fi;
}
...
function downloadGoogleDrive() {
  ...
  if  [ $_DOWNLOAD_GDRIVE_EXIT_CODE -ne 0 ]; then
    printf "\e[91m\xE2\x9C\x95\e[39m\n" >&2
    printf "\n$_MSG_ERROR_DOWNLOADING_GDRIVE (v$_GDRIVE_LASTEST_VERSION)\n" >&2
    printf "$_MSG_URL: $_DOWNLOAD_GDRIVE_URL\n" >&2
    printf "$_MSG_ERROR: $_DOWNLOAD_GDRIVE_OUTPUT\n" >&2
  ...
  if [ "$_GDRIVE_BINARY_CONTENT" = "Not Found" ]; then
    ...
    if [ $_GET_GDRIVE_VERSION_404_ATTEMPTS -ge $_GET_GDRIVE_VERSION_404_MAX_ATTEMPTS ]; then
      ...
      printf "$_MSG_ERROR: $_GDRIVE_BINARY_CONTENT\n" >&2
      exit 404
    else
...
function main() {
  ...
  printf "$_MSG_CHECKING_GDRIVE_CLIENT\n"
  ...
  if [ "$_GDRIVE_FILEPATH" = "" ]; then
    ...
    printPrependedStdout
    printf "  $_MSG_DOWNLOADING_GDRIVE_CLIENT (v$_GDRIVE_LASTEST_VERSION)..."
    ...
    if [ $_GDRIVE_VERSION_OUTPUT_EXIT_CODE -ne 0 ]; then
      ...
      printPrependedStdout
    	printf "  $_MSG_FOUND_GDRIVE_INSTALLED (v$_GDRIVE_INSTALLED_VERSION)"
      printf " \e[92m\xE2\x9C\x94\e[39m\n"

      # Si no está actualizado a la última versión
      if [ "$_GDRIVE_INSTALLED_VERSION" != "$_GDRIVE_LASTEST_VERSION" ]; then
        printPrependedStdout
        printf "  $_MSG_UPDATING_GDRIVE (v$_GDRIVE_INSTALLED_VERSION"
        printf " -> v$_GDRIVE_LASTEST_VERSION)..."
