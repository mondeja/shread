...
find src -type f -name '*.sh' | while read script_filepath; do
  ...
  for lang in "${SUPPORTED_LANGUAGES[@]}"; do
    ...
    if [ "$lang" != "en" ]; then
      while IFS= read -r line; do
        ...
        if [[ $line = _MSG* ]]; then
          ...
          # Get msgstr
          MSGSTR_LINE=$(
            cat $po_filepath | grep -A 1 "msgid \"$MSGID\"" | tail -n 1)
    ...
    echo "source <(printf '$(base64 $temp_localized_script)' | base64 -d)" \
      >> $path_to_script_dest
